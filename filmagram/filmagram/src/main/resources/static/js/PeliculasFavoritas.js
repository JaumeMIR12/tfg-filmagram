const PeliculasFavoritas = {
    ...
    loadPeliculasFavoritas: function(usuarioId) {
    ...
    view: () => {
        ...
        if (PeliculasFavoritas.peliculas.length === 0) {
            return m('div', 'Aún no tienes ninguna película como favorita.');
        }

        if (!PeliculasFavoritas.peliculas) {
            PeliculasFavoritas.loadPeliculasFavoritas();
            return m('div', 'Cargando...');
        }
        
        return m('section', { id: 'seccion1', class: 'section col-pelis col-main' }, [
        m("div", { class: "poster-grid" }, [
            m("ul", { class: "grid -grid -p125 -constrained clear" }, PeliculasFavoritas.peliculas.map(function(pelicula) {
...
PeliculasFavoritas.loadPeliculasFavoritas(usuarioId);

// Montar el componente en el elemento con id "app"
document.addEventListener('DOMContentLoaded', () => {
            // Monta el componente en el elemento con el ID "app"
            m.mount(document.getElementById('app'), PeliculasFavoritas);
        });