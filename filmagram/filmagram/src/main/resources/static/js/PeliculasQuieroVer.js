const PeliculasQuieroVer = {
    // Estado inicial del componente
    peliculas: [],

    // Función para cargar los datos del director
    loadPeliculasQuieroVer: function(usuarioId) {
        ...
        m.request({
            ...
            PeliculasQuieroVer.peliculas = response;
            console.log(response);
            m.redraw(); // Forzar un nuevo renderizado después de recibir datos
        }).catch(function(error) {
            console.error("Error al cargar películas watchlist de usuario:", error);
        });
    },
    ...
    view: () => {
        ...
        if (PeliculasQuieroVer.peliculas.length === 0) {
            return m('div', 'Aún no tienes ninguna película en esta lista.');
        }

        if (!PeliculasQuieroVer.peliculas) {
            PeliculasQuieroVer.loadPeliculasQuieroVer();
            return m('div', 'Cargando...');
        }
        
        return m('section', { id: 'seccion1', class: 'section col-pelis col-main' }, [
        m("div", { class: "poster-grid" }, [
            m("ul", { class: "grid -grid -p125 -constrained clear" }, PeliculasQuieroVer.peliculas.map(function(pelicula) {
...
PeliculasQuieroVer.loadPeliculasQuieroVer(usuarioId);

// Montar el componente en el elemento con id "app"
document.addEventListener('DOMContentLoaded', () => {
            // Monta el componente en el elemento con el ID "app"
            m.mount(document.getElementById('app'), PeliculasQuieroVer);
        });